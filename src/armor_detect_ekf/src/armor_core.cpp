    ...
    : env_(ORT_LOGGING_LEVEL_WARNING, "yolov8")
{
    ...
    auto configure_session_options = [](Ort::SessionOptions& options)
    {
    ...
    if (use_cuda)
    {
        try
        {
            ...
            configure_session_options(cuda_session_options);

            OrtCUDAProviderOptions cuda_options{};
            cuda_options.device_id = 0;
            cuda_session_options.AppendExecutionProvider_CUDA(cuda_options);

            // 优先尝试 CUDA 推理
            session_ = Ort::Session(env_, model_path.c_str(), cuda_session_options);
            cuda_enabled = true;
        }
        catch (const Ort::Exception& e)
        {
            // CUDA 不可用则自动回退到 CPU
            std::cerr << "[ArmorDetector] CUDA EP unavailable, fallback to CPU: "
                      << e.what() << std::endl;
        }
    }
    ...
    std::cout << "[ArmorDetector] Model loaded with "
              << (cuda_enabled ? "CUDA" : "CPU")
              << ". Input: " << input_w_ << "x" << input_h_ << std::endl;
}
