...
{
  // 构造函数: 选择检测模式 (YOLO/传统CV), 配置发布者/订阅者/调试工具
  ...
  // 传统CV检测器初始化: 读取二值化/灯条/装甲板/分类器参数
{
  ...
  // YOLO检测器初始化: 加载OpenVINO模型，配置推理参数
std::unique_ptr<YoloDetector> ArmorDetectorNode::initYoloDetector()
{
  ...
  // 检测调度: 根据 use_yolo_ 选择YOLO或传统CV模式，按detect_color过滤结果
