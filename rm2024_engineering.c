...
arm_state_t arm_state = ARM_IDLE;
...
{
case ARM_IDLE:
    memcpy(set_pos6, grab_pre, sizeof(grab_pre));
    arm_state = ARM_MOVE_PRE;
    break;

case ARM_MOVE_PRE:
    i_kin(set_pos6, joints, 'V');
    if (arm_reached(joints))
        arm_state = ARM_MOVE_DOWN;
    break;

case ARM_MOVE_DOWN:
    memcpy(set_pos6, grab_real, sizeof(grab_real));
    i_kin(set_pos6, joints, 'V');
    if (arm_reached(joints))
        arm_state = ARM_GRAB;
    break;
...
case ARM_GRAB:
    // 如果有夹爪 motor
    //wb_motor_set_position(gripper_motor, 0.0); // 闭合
    wb_robot_step(500); // 给夹爪时间
    arm_state = ARM_LIFT;
    break;

case ARM_LIFT:
    memcpy(set_pos6, grab_lift, sizeof(grab_lift));
    i_kin(set_pos6, joints, 'V');
    if (arm_reached(joints))
        arm_state = ARM_DONE;
    break;
...
while (wb_robot_step(TIME_STEP) != -1)
{
    ...
    for (int i = 0; i < JOINT_NUM; i++)
    {
        double step = (i == 0) ? 0.001 : 0.01;
        if (fabs(joints[i].now - joints[i].set) > step)
        {
            if (joints[i].now < joints[i].set)
                joints[i].out = joints[i].now + step;
            else
